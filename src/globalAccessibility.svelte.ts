...
class GlobalAccessibilityStore {
  ...
  #evaluationInterval: ReturnType<typeof setInterval> | null = null;
  ...
  score = $derived.by(() => {
    if (this.#stats.evaluatedElements === 0) return 100;
  ...
  #backpressureState: 'normal' | 'high' = 'normal';

  private handleResults = (newResults: EvaluationResult[], newStats: EvaluationStats) => {
  ...
  private clearTransmittedResults(count: number) {
  ...
  private startContinuousEvaluation() {
    ...
    this.#evaluationInterval = setInterval(() => {
      if (this.#orchestrator && this.#config.enabled) {
        this.#orchestrator.evaluate().catch((error: unknown) => {
  ...
  async evaluate() {
    if (!this.#orchestrator || this.#isAnalyzing) return;
  ...
  private estimateBufferMemoryUsage(): number {
    const avgResultSizeBytes = 500;
    const totalBytes = this.#results.length * avgResultSizeBytes;
    return totalBytes / (1024 * 1024);
  }

  private emitBackpressure(level: 'normal' | 'high' | 'critical', bufferSize: number) {
    if (!this.#orchestrator?.client) return;
