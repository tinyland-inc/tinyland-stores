...
class ThemeStore {
    ...
    #themes = $state([]);
    #loadedThemes = $state(new Set(['trans']));
    ...
    async init() {
        ...
        const defaultDarkModePreference = fallbackDarkMode || 'dark';
    ...
    setupHotReload() {
        window.addEventListener('theme-refresh', async (event) => {
